...
class QuestionRepository extends AbstractRepository{
    ...
    public function construire(array $ut): Question {
         return new Question($ut["author"], $ut["private"], $ut["title"], $ut["description"], $ut["dateStart"], $ut["dateEnd"], $ut["dateCreation"]);
    }
    ...
    public static function getBestAnwsersByQuestion(int $idQuestion): array {
        ...
        foreach ($sections as $section) {
            ...
            foreach ($answers as $answer) {
                $nb = AnswerRepository::getAllVotes($answer->getIdAnswer());
                if ($nb > $maxVotes) {
    ...
    public static function getIdWithidSection($idsection){
        $sql="SELECT idQuestion FROM Sections s WHERE idSection=:idSectionTag";
        $pdoStatement=DatabaseConnection::getPdo()->prepare($sql);
        $value=array (
            "idSectionTag"=>$idsection
        );
